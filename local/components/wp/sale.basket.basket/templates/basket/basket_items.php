...
<?
				...
				foreach ($arResult["GRID"]["ROWS"] as $k => $arItem) {
					$currentCompanyId = $arItem['PROPS'][0]['COMPANY_ID'];
					if ($currentCompanyId != $prevCompanyId && !$first) { ?>
						<tr class='buttonBuy' id='skip'>
							<td colspan='7'>
								...
								{
									?>
									<a href="javascript:void(0)" onclick="updateBasket();" style='float: right;' class="btn btn-blue-full checkout refresh"><?=GetMessage("SALE_REFRESH")?></a>
									<?
								}
								?>
								<a href="javascript:void(0)" id='<? echo $prevCompanyId; ?>' onclick="checkOut(this);" style='float: right;' class="btn btn-blue-full checkout"><?=GetMessage("SALE_ORDER")?></a>
							</td>
						</tr>
<?
					}
					...
					$prevCompanyId = $arItem['PROPS'][0]['COMPANY_ID'];
					$first = false;
				} // endforeach;
				?>
