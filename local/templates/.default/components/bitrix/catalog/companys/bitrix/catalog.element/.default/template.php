...
//pre($arResult);
// Проверим, находится карточка на модерации или нет.
if ('1' == $arResult['PROPERTIES']['inModeration']['VALUE'])
{
?>
<div class="block-default in block-shadow content-margin corpnewsblock corpnewsblockdetal">
	<div class="newsbitem clearfix">
		Информация компании <? echo $name; ?> редактируется.
	</div>
</div>
<?
	return true;
	...
	if ($arResult['DETAIL_PICTURE']['SRC'])
		$file = CFile::ResizeImageGet($arResult['DETAIL_PICTURE']["ID"], array('width'=>160, 'height'=>100), BX_RESIZE_IMAGE_PROPORTIONAL, true);
	elseif ($arResult['PREVIEW_PICTURE']['SRC'])
		$file = CFile::ResizeImageGet($arResult['PREVIEW_PICTURE']["ID"], array('width'=>160, 'height'=>100), BX_RESIZE_IMAGE_PROPORTIONAL, true);
	else
		$file['src'] = EMPTY_IMAGE_PATH;
	
	$companyId = $arResult['ID'];

	//echo $DB->FormatDate("31.12.2005", "DD.MM.YYYY", "DD-MMMM-YYYY");
	$timeStamp = FormatDate("d F Y", MakeTimeStamp($arResult['TIMESTAMP_X']));

	$city = $contactPerson = $userCity = null;
	foreach ($arResult['DISPLAY_PROPERTIES'] as $code => $displayProperty)
	{
		//pre($displayProperty);
	...
	$country = $arResult['PROPERTIES']["country"]['VALUE'];
	...
	$staff = $users = null;
	if (is_array($arResult['PROPERTIES']['staff']['VALUE']) && !empty($arResult['PROPERTIES']['staff']['VALUE'][0]))
	{
		foreach ($arResult['PROPERTIES']['staff']['VALUE'] as $key => $userId)
			$users .= $userId . ' | ';

		$staff = substr($users, 0, -2);
	}
	...
	if (null !== $staff && !empty($staff))
	{
		$arFilter = array('ID' => $staff);
		$arRes = CUser::GetList($order, $tmp, $arFilter);
		$tmpStr = '';
		while ($res = $arRes->Fetch())
			$tmpStr .= $res['NAME'] . ' ' . $res['LAST_NAME'] . ', ';

		$staff = substr($tmpStr, 0, -2);
	}
	...
	// Вытащим свойства привязанных элементов->прайс-листов
	$fileIdArray = array();
	// $fileArr = $arResult["PROPERTIES"]["priceListFile"]["VALUE"];
	// foreach ($fileArr as $key => $fileId)
	// {
		// $arFilter = Array("IBLOCK_ID" => IBLOCK_ID_PRICE_LISTS, "ID" => $fileId);
		$arFilter = Array("IBLOCK_ID" => IBLOCK_ID_PRICE_LISTS, "PROPERTY_companyId" => $arResult['ID'], 'ACTIVE' => 'Y');
		$res = CIBlockElement::GetList(Array(), $arFilter);
		while ($ob = $res->GetNextElement())
		{
			//$arFields = $ob->GetFields(); // поля элемента
			$arProps = $ob->GetProperties(); // свойства элемента
			array_push($fileIdArray, $arProps['file']['VALUE']);
		}
	// }
	...
	{
		$fileProp = CFile::GetFileArray($fileId);
		// pre($fileProp);
		$fileSize = ($fileProp['FILE_SIZE'] > 1000)? ceil($fileProp['FILE_SIZE'] / 1000) . ' Кб': $fileProp['FILE_SIZE'] . ' б';
		$priceList = array('src' => $fileProp['SRC'], 'name' => $fileProp['ORIGINAL_NAME'], 'size' => $fileSize);
...
<? if (isset($_GET['message'])) { ?>
	<div>
		<h3 style='color:green;'>Сообщение отправлено администраторам компании</h3>
	</div>
<?}?>
	<div class="newsbitem clearfix">
		<div class="newsbimg floatleft text-center">
			<img src="<? echo $file['src']; ?>">
			<div class="newsbtop100">
				Позиция в <a href="/top100/">Топ-100</a>: <? echo $arResult["PROPERTIES"]["placeInRating"]["VALUE"]; ?>
			</div>
		</div>
		<div class="newsbtext">
			<h1><? echo $name; ?></h1>
			<div class="tagblock clearfix" id='menu'>
				<span class="gtag floatleft">Последнее обновление: <? echo $timeStamp; ?></span>
				<?
					...
					if ($USER->IsAuthorized())
					{
						...
						// Если пользователь не сотрудник этой компании, то он может написать в эту компанию.
						if (!empty($arUser['UF_ID_COMPANY']) && ($arResult['ID'] != $arUser['UF_ID_COMPANY']))
						{
				?>
							<a class="btag floatright" href='/company/writetocompany/?companyId=<? echo $arResult['ID']; ?>&detailPage=<? echo $arResult['DETAIL_PAGE_URL']; ?>'>
								Написать в компанию
							</a>
				<?		}
						elseif (empty($arUser['UF_ID_COMPANY']))
						{
				?>
							<div class="btag floatright addStaffInCompany" id='<? echo $arResult['ID']; ?>'>Я сотрудник</div>
							<a class="btag floatright" href="#" id='sendMessage'>
								Написать в компанию
							</a>
				<?		}
						elseif ($arResult['ID'] == $arUser['UF_ID_COMPANY'])
						{
							// По хорошему сюда надо добавить ещё проверку на существование в списке сотрудников компании...
							...
							{
				?>
								<div class="btag floatright">Вы сотрудник в этой компании</div>
				<?			}
							else
							{
				?>
								<div class="btag floatright">Ваш запрос на модерации</div>
				<?
							}
						}
					}
	...
	<script>
		function successReuest()
		{
			$('.addStaffInCompany').remove();
			$('#sendMessage').remove();
			$('#menu').append('<div class="btag floatright">Запрос отправлен на модерацию</div>');
		}
	
		$('.addStaffInCompany').on('click', function(){
			var companyId = $(this).attr('id');
			$.ajax({
				type: 'POST',
				dataType: 'json',
				url: '/formsActions/',
				data: {companyId: companyId, actionNum: 3},
	...
	<h3 class="bltitle">Визитная карточка компании</h3>
	<div class="panel panel-default">
		<table class="table table-striped table-hover table-bordered block-default tableblock">
			<tbody>
				<tr>
					<td class="table256">Категория:</td>
					<td><? echo $categoriesString; ?>	</td>
				</tr>
				<tr>
					<td class="table256">Страна/ Город:</td>
					<td><? echo !empty($country)? $country . ', ' . $city: $city; ?></td>
				</tr>
				<tr>
					<td class="table256">Адрес:</td>
					<td><? echo $arResult['PROPERTIES']['address']['VALUE']; ?></td>
				</tr>
<?
				if (isset($arResult['PROPERTIES']['addressAddOffices']['VALUE']) && !empty($arResult['PROPERTIES']['addressAddOffices']['VALUE'])) {
					$addressStr = '';
					foreach ($arResult['PROPERTIES']['addressAddOffices']['VALUE'] as $key => $value) {
						$addressStr .= $value . ', ';
					}

					$addressStr = substr($addressStr, 0, -2);
?>
					<tr>
						<td class="table256">Дополнительные адреса:</td>
						<td><? echo $addressStr; ?></td>
					</tr>
					<?
				}
?>
...
?>
					<tr>
						<td class="table256">Контактное лицо:</td>
						<td>
						<?
							foreach ($contactPerson as $key => $user)
							{
								if ((int)$count > 1)
								{
									?><a href="/users/<? echo $user['id']; ?>.html"><? echo $user['name']; ?>, </a><?
								}
								else
								{
									?><a href="/users/<? echo $user['id']; ?>.html"><? echo $user['name']; ?></a><?
...
<?
				...
				if (null !== $staff) { ?>
					<tr>
						<td class="table256">Сотрудники компании:</td>
						<td>
							<? echo $staff; ?>
						</td>
					</tr>
...
?>
				...
				<tr>
					<td class="table256">Телефон:</td>
					<td><a href="tel:<? echo $arResult['PROPERTIES']['phone']['VALUE']; ?>"><? echo $arResult['PROPERTIES']['phone']['VALUE']; ?></a></td>
				</tr>
<?
				if (isset($arResult['PROPERTIES']['addPhone']['VALUE']) && !empty($arResult['PROPERTIES']['addPhone']['VALUE'])) {
					$phoneStr = '';
					foreach ($arResult['PROPERTIES']['addPhone']['VALUE'] as $key => $value) {
						$phoneStr .= $value . ', ';
					}
...
?>
					<tr>
						<td class="table256">Дополнительные телефоны:</td>
						<td><? echo $phoneStr; ?></td>
					</tr>
					<?
				}
?>
				<tr>
					<td class="table256">Факс:</td>
					<td><? echo $arResult['PROPERTIES']['fax']['VALUE']; ?></td>
				</tr>
				<tr>
					<td class="table256">E-mail:</td>
					<td><a href="mailto:<? echo $arResult['PROPERTIES']['email']['VALUE']; ?>"><? echo $arResult['PROPERTIES']['email']['VALUE']; ?></a></td>
				</tr>
				<tr>
					<td class="table256">Сайт компании:</td>
					<td><a href="http://<? echo $arResult['PROPERTIES']['companyWebsite']['VALUE']; ?>"><? echo $arResult['PROPERTIES']['companyWebsite']['VALUE']; ?></a></td>
				</tr>
<?
				if (isset($arResult['PROPERTIES']['skype']['VALUE']) && !empty($arResult['PROPERTIES']['skype']['VALUE'])) { ?>
					<tr>
						<td class="table256">Skype:</td>
						<td><? echo $arResult['PROPERTIES']['skype']['VALUE']; ?></td>
					</tr>
<?				}
// pre($arResult['PROPERTIES']);
				if (isset($arResult['PROPERTIES']['socialNetworksVK']['VALUE']) && !empty($arResult['PROPERTIES']['socialNetworksVK']['VALUE'])) { ?>
					<tr>
						<td class="table256">VK:</td>
						<td><? echo $arResult['PROPERTIES']['socialNetworksVK']['VALUE']; ?></td>
					</tr>
<?				}
				
				if (isset($arResult['PROPERTIES']['socialNetworksFB']['VALUE']) && !empty($arResult['PROPERTIES']['socialNetworksFB']['VALUE'])) { ?>
					<tr>
						<td class="table256">Facebook:</td>
						<td><? echo $arResult['PROPERTIES']['socialNetworksFB']['VALUE']; ?></td>
					</tr>
<?				}

				if (isset($arResult['PROPERTIES']['socialNetworksGoole']['VALUE']) && !empty($arResult['PROPERTIES']['socialNetworksGoole']['VALUE'])) { ?>
					<tr>
						<td class="table256">Google+:</td>
						<td><? echo $arResult['PROPERTIES']['socialNetworksGoole']['VALUE']; ?></td>
					</tr>
<?				}

				if (isset($arResult['PROPERTIES']['socialNetworksInstagram']['VALUE']) && !empty($arResult['PROPERTIES']['socialNetworksInstagram']['VALUE'])) { ?>
					<tr>
						<td class="table256">Instagram:</td>
						<td><? echo $arResult['PROPERTIES']['socialNetworksInstagram']['VALUE']; ?></td>
					</tr>
<?				}

				if (isset($arResult['PROPERTIES']['conditionEmployment']['VALUE']) && !empty($arResult['PROPERTIES']['conditionEmployment']['VALUE'])) { ?>
					<tr>
						<td class="table256">Условия работы:</td>
						<td><? echo $arResult['PROPERTIES']['conditionEmployment']['VALUE']; ?></td>
					</tr>
<?				}
?>
	...
	<?
	if (!empty($priceListsArray))
	{
?>
		<h3>Прайс-лист</h3>
		<div class="pricelistblock clearfix">
<?
			foreach ($priceListsArray as $key => $file)
			{
?>
				<div class="pricelistdown floatleft">
						<div class="pricelistimg xls floatleft">
							<a href="<? echo $file['src']; ?>">
								<?
								/*
						...
						<div class="pricelisttext">
							<div class="pricelistlink">
								<a href="<? echo $file['src']; ?>"><? echo $file['name']; ?></a>
							</div>
...
<?
	$GLOBALS['arrFilter'] = array("!PROPERTY_TOINDEX" => false, 'PROPERTY_companyId' => $companyId, 'PROPERTY_paidOption_VALUE' => 1);
	$APPLICATION->IncludeComponent("bitrix:news.index", "paidSectionsInDetailCompany", Array(
		"IBLOCKS" => array(	// Код информационного блока
				0 => IBLOCK_ID_BRANDS,
				1 => IBLOCK_ID_LICENSE,
				2 => IBLOCK_ID_CATALOGS_PDF
			),
			...
			"COMPONENT_TEMPLATE" => "paidSectionsInDetailCompany",
