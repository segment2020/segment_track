...
<?
if (!CSite::InDir('/bannerstatistic/'))
{
?>
	<h2>Статистика</h2>
<?}?>
...
<?
	...
	if ($arResult["DETAIL_PICTURE"])
		...
		$imgDetail = CFile::ResizeImageGet($arResult["DETAIL_PICTURE"]["ID"], array('width' => 1024, 'height' => 768), BX_RESIZE_IMAGE_PROPORTIONAL, true); //BX_RESIZE_IMAGE_EXACT
	else
		$imgDetail['src'] = '';

	$showCounter = showviews($arResult['ID']);

	foreach ($arResult['PROPERTIES']['hostingPage']['VALUE'] as $key => $pageName)
	{
		$hostingPage .= $pageName . ', ';
	}
	$hostingPage = substr($hostingPage, 0, -2);
...
*/
	...
	$clicksNum = getClicksNumber($arResult['ID']);
	if (empty($clicksNum) || false === $clicksNum)
		$clicksNum = 0;

	if ($clicksNum != 0)
		$ctr = number_format(($clicksNum / $showCounter ) * 100, 4);
	else
		$ctr = 0;
...
<?		} ?>
		<div class="aboutevent">
			...
			<div class="descrdate">
				...
				<span class="dname">Кликов:</span><span class="ddate"><? echo $clicksNum; ?></span>
			</div>
			<div class="descrline">
				<span class="lname">CTR:</span><span class="ldescr"><? echo $ctr; ?></span><br />
				<span class="lname">Ссылка:</span><span class="ldescr"><a href='<? echo $arResult['PROPERTIES']['link']['VALUE']; ?>' target='_blank'><? echo $arResult['PROPERTIES']['link']['VALUE']; ?></a></span><br />
