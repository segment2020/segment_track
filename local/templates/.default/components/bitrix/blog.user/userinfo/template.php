				...
				<div class="newsbimg floatleft text-center">
					<?
						...
						if (!empty($arResult['User']['PERSONAL_PHOTO']))
							$avatarFile = CFile::ResizeImageGet($arResult['User']['PERSONAL_PHOTO'], array('width'=>160, 'height'=>160), BX_RESIZE_IMAGE_PROPORTIONAL, true);
						else
							$avatarFile['src'] = EMPTY_LOGO_AVATAR_PATH;	
						
					?>			
					<img src="<?=$avatarFile['src']?>">
				</div>
				<div class="newsbtext">
					<h1><?=$arResult['userName']?></h1>
					<div class="tagblock clearfix" id="menu">
						<span class="gtag floatleft">Последнее обновление: <?=$arResult['User']['TIMESTAMP_X']?></span>
					</div>
				</div>
			</div>
			<div class="seporator"></div>
			<h3 class="bltitle">Профиль</h3>
			<div class="panel panel-default">
				<table class="table table-striped table-hover table-bordered block-default tableblock">
					<tbody>
						<?if(strlen($arResult["User"]["PERSONAL_COUNTRY"])>0) {?>
						<tr>
							<td class="table256">Страна/ Город:</td>
							<td><?=GetCountryByID($arResult["User"]["PERSONAL_COUNTRY"], "ru");?> <?=$arResult["User"]["PERSONAL_CITY"]?></td>
						</tr>
						<? } ?>
						<?if(strlen($arResult["USER_PROPERTIES"]["DATA"]["UF_ID_COMPANY"]["ID"])>0) {?>
						<tr>
							<td class="table256">Компания:</td>
							<td>
								<?$APPLICATION->IncludeComponent(
										"bitrix:system.field.view", 
										$arResult["USER_PROPERTIES"]["DATA"]["UF_ID_COMPANY"]["USER_TYPE"]["USER_TYPE_ID"], 
										array("arUserField" => $arResult["USER_PROPERTIES"]["DATA"]["UF_ID_COMPANY"]), null, array("HIDE_ICONS"=>"Y"));?>						
							</td>
						</tr>
						<? } ?>						
						<?if(strlen($arResult["User"]["WORK_POSITION"])>0) {?>
						<tr>
							<td class="table256">Должность:</td>
							<td><?=$arResult["User"]["WORK_POSITION"];?></td>
						</tr>
						<? } ?>	 
						<?if(strlen($arResult["User"]["PERSONAL_PHONE"])>0) {?>
							<tr>
						...
						<tr>
							...
							<td><?=$arResult["User"]["PERSONAL_ICQ"];?></td>
						</tr>
						<? } ?>				
						<?if(strlen($arResult["User"]["PERSONAL_WWW"])>0) {?>
						<tr>
							<td class="table256">Сайт:</td>
