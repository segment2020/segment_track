...
if (!empty($arResult["DETAIL_PICTURE"]["SRC"]))
	$file = CFile::ResizeImageGet($arResult["DETAIL_PICTURE"]["ID"], array('width'=>160, 'height'=>160), BX_RESIZE_IMAGE_EXACT, true);

if (isset($arResult['PROPERTIES']['FORUM_MESSAGE_CNT']['VALUE']) && !empty($arResult['PROPERTIES']['FORUM_MESSAGE_CNT']['VALUE']))
		$msgCounter = $arResult['PROPERTIES']['FORUM_MESSAGE_CNT']['VALUE'];
	else
		$msgCounter = 0;
?>
...
<div class="block-default in block-shadow content-margin detailblock">
	...
	<div class="detailinfo clearfix">
		<div class="detailinfofirm floatleft">
			Мнение компании <? if ($arResult['DISPLAY_PROPERTIES']['companyId']['DISPLAY_VALUE']) { echo $arResult['DISPLAY_PROPERTIES']['companyId']['DISPLAY_VALUE']; } ?></a>
		</div>
		<div class="detailinfolink floatleft">
			<a href="<?=$arResult['IBLOCK']['SECTION_PAGE_URL']?>?companyId=<? echo $arResult['PROPERTIES']['companyId']['VALUE']; ?>"><i class="icon-icons_main-10"></i><span>Все мнения компании</span></a>
		</div>
	</div>
	<? } ?>

	<div class='printblock'>
	<div class="opiniontitleblock clearfix">
		<? if ($file['src']) { ?>
		<div class="opiniontitleimg floatleft">
			<img src="<? echo $file['src']; ?>" />
		</div>
		<? } ?>
		<div class="opiniontitledescr <? if (!$file['src']) { echo 'nomarginleft'; } ?>">
			<div class="opiniontitlename"><? echo $arResult['PROPERTIES']['name']['VALUE']; ?>:</div>
			...
			<div class="infotvc">
				...
				<!--<span class="toprint"><a href='javascript:window.print(); void 0;'>На печать</a></span>-->
				<span class="toprint"><a class="printBlock" href="#">На печать</a></span>
			</div>							
		</div>
	</div>
