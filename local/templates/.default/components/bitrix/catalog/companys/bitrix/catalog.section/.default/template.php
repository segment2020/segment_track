...
<h1>Участники рынка</h1>
	<div class="addfilterblock clearfix content-margin">
		<div class="addcompanyblock floatleft">
			<div class="btn btn-blue-full minbr"><span class="plus">+</span>Добавить свою компанию</div>
		</div>
	...
	<div class="block-default in block-shadow content-margin advancedsearchblock">
		...
		<form action='<? echo $page; ?>' method='GET' class='' id='companyFilterForm'>
			<div class="row">
				<div class="col-xs-4">
					<div class="form-group">
						<label class="control-label mainlabel" for="regionsList">Регион</label>
...
?>
				...
				<div class="col-xs-4">
					<div class="form-group hide" id='areaListBlock'>
						<label class="control-label mainlabel" for="areaList">Область</label>
				...
				<div class="col-xs-4">
					<div class="form-group hide" id='citiesListBlock'>
						<label class="control-label mainlabel" for="citiesList">Город</label>
			...
			<div class="row">
				...
				<div class="col-xs-6">
					<div class="form-group">
						...
						<select class="selectpicker selectboxbtn form-control minbr" name='typeCompanyId' id='typeCompanyId'>
						<option value='' id='/company/list/'>Bce</option>
<?
							$blockId = 1;
							//выберем папки из информационного блока $blockId и раздела 0.
							$items = GetIBlockSectionList($blockId, 0, array("sort" => "asc"));
							...
							{
								$selected = '';
								if ($arItem["ID"] == $arResult['ORIGINAL_PARAMETERS']['SECTION_ID'])
									$selected = 'selected';

								echo '<option value="' . $arItem["ID"] . '" id="' . $arItem['SECTION_PAGE_URL'] . '/" ' . $selected . '>' . $arItem["NAME"] . '</option>';
							}
?>
						</select>
					</div>
	...
	<div class="block-default block-shadow content-margin fliteralphabetblock clearfix show_rus">				
		...
		<div class="fliteralphabetshowall floatright">
			<a href="/company/list" id='showAllcompany'><i class="icon-icons_main-05"></i>Показать все</a>
		</div>					
...
<script>
	$('#typeCompanyId').on('change', function(){
		var id = $("option:selected", this).attr('id');
		console.log(id);
		$('#companyFilterForm').attr('action', id);
	});
</script>
