...
foreach($arResult["ITEMS"] as $arItem)
{
	...
	$showCounter = showviews($arItem['ID']);


	$file['src'] = '/tpl/images/pdfIcon90x90.png';

	if (!empty($arItem['PROPERTIES']['companyId']['VALUE']))
	{
	...
	$displayProp = true;
	$money = false;
	if (!empty($arItem['PROPERTIES']['paidOption']['VALUE']) && !$money)
	{
...
<?
						...
						{
?>
							<span class="infocomment">Страна <? echo GetCountryByID($arItem['PROPERTIES']['country']['VALUE']); ?></span>
<?						}
?>
					</div>
				</div>
...
<?
			if (!empty($arItem['DISPLAY_PROPERTIES']['companyId']['DISPLAY_VALUE']) && $displayProp)
			{
?>
				<div class="newsbfirm">Каталог компании <? echo $arItem['DISPLAY_PROPERTIES']['companyId']['DISPLAY_VALUE']; ?></div>
		<?	}
		?>
		</div>
