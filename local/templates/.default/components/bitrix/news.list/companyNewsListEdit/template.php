...
<?}
// pre($arResult);
if (IBLOCK_ID_NOVETLY == $arResult['ITEMS'][0]['IBLOCK_ID'])
	$link = '/personal/company/new/edit/?iBlockId=' . $arResult['ID'] . '&iBlockType=' . $arResult['IBLOCK_TYPE_ID'];
else
	$link = '/personal/company/news/edit/?iBlockId=' . $arResult['ID'] . '&iBlockType=' . $arResult['IBLOCK_TYPE_ID'];
?>
...
if (count($arResult["ITEMS"]) > 0){
	...
	$file['src'] = EMPTY_IMAGE_PATH; 
		if ($arElement["PREVIEW_PICTURE"]["SRC"])
			$file = CFile::ResizeImageGet($arElement["PREVIEW_PICTURE"]["ID"], array('width'=>160, 'height'=>160), BX_RESIZE_IMAGE_PROPORTIONAL, true);
		else
		{
			if (IBLOCK_ID_NOVETLY != $arParams['IBLOCK_ID'])
			{
				if ('1' === $arElement['PROPERTIES']['showLogo']['VALUE'])
				{
					$arSelect = array("PREVIEW_PICTURE");
					$arFilter = array("IBLOCK_ID" => IBLOCK_ID_COMPANY, 'ID' => $arElement['PROPERTIES']['companyId']['VALUE'], "ACTIVE" => "Y");
	...
	if ($fileNum)
		$file = CFile::ResizeImageGet($fileNum, array('width'=>160, 'height'=>160), BX_RESIZE_IMAGE_PROPORTIONAL, true);
	...
	$pubRej = (isset($arElement['PROPERTIES']['PUB_REJECTED']['VALUE']))? $arElement['PROPERTIES']['PUB_REJECTED']['VALUE']: $arElement['PROPERTIES']['rejected']['VALUE'];

	$rejMessage = '';
	if ('Y' == $pubRej || '1' == $pubRej) {
		$status = 'Отклонена';
		$statusblock = 'status_r';
		$rejMessage = (isset($arElement['PROPERTIES']['REASON_FOR_REJ']['VALUE']['TEXT']))? $arElement['PROPERTIES']['REASON_FOR_REJ']['VALUE']['TEXT']: $arElement['PROPERTIES']['reasonRejection']['VALUE']['TEXT'];
...
?>
	...
	<div class='row'>
		<a href="/personal/company/news/edit/?iBlockId=<? echo $arResult['ID']; ?>&iBlockType=<? echo $arResult['IBLOCK_TYPE_ID']; ?>">
			<div class='col-xs-12 btn btn-blue-full minbr'>
				<span class="plus">+</span>
				<?=GetMessage("ADD_NEWS")?>
			</div>
