		...
		if ($arParams['DISPLAY_NAME'] === 'Y')
		{
			...
			if ($arResult['LABEL'] && !empty($arResult['LABEL_ARRAY_VALUE']))
			{
				foreach ($arResult['LABEL_ARRAY_VALUE'] as $code => $value)
				{
					if ('Хит' === $value)
					{
						// pre($arResult['IBLOCK_ID']);
						$companyName = strip_tags($arResult['DISPLAY_PROPERTIES']['companyId']['DISPLAY_VALUE']);
						$value = !empty($companyName)? '<a href="/hits/?companyId=' . $arResult['PROPERTIES']['companyId']['VALUE'] . '"><span class="redText">Хит компании</span> ' . $companyName . '</a>': 'Хит';
						break;
					}
				}
...
?>
			<div class="row">
				<div class="col-xs-12">
					<h1 class="bx-title" style='padding-left: 7px;'><? echo $nameH1; ?></h1>
				</div>
			</div>
			<?
		}
		...
		<div class="row">
			<div class="col-md-2 col-sm-12 slider-inner">
<?
				if ($arResult['PROPERTIES']['additionalPhoto']['VALUE'])
				{
?>
					<div class="gallery">
						<div class="button-prev"><i class="icon-icons_main-10"></i></div>
						<div class="swiper-container">
							<div class="swiper-wrapper">
								<div class="swiper-slide">
									<div class="img-wrapper"><img src="<? echo $arResult['DETAIL_PICTURE']['SRC']; ?>" class='imgClick' alt=""></div>
								</div>
<?
								foreach ($arResult['PROPERTIES']['additionalPhoto']['VALUE'] as $key => $imgId)
								{
?>
									<div class="swiper-slide">
										<div class="img-wrapper"><img src="<? echo CFile::GetPath($imgId); ?>" class='imgClick' alt=""></div>
									</div>
<?								}
?>
							</div>
						</div>
						<div class="button-next"><i class="icon-icons_main-10"></i></div>
					</div>
<?				}
?>
			</div>
			<div class="col-md-6 col-sm-12 full-image-preview">
				<div class="product-item-detail-slider-container" id="<?=$itemIds['BIG_SLIDER_ID']?>" style='height: 100%;'>
					<span class="product-item-detail-slider-close" data-entity="close-popup"></span>
					<div class="product-item-detail-slider-block
						<?=($arParams['IMAGE_RESOLUTION'] === '1by1' ? 'product-item-detail-slider-block-square' : '')?>" data-entity="images-slider-block" style='height: 98%;'>
						<span class="product-item-detail-slider-left" data-entity="slider-control-left" id='slideLeft' data-slideid='' ></span>
						<span class="product-item-detail-slider-right" data-entity="slider-control-right" id='slideRight' data-slideid='<? echo $arResult['PROPERTIES']['additionalPhoto']['VALUE'][0]; ?>'></span>
						...
						?>
						<div class="product-item-detail-slider-images-container" data-entity="images-container" id='mainContainer'>
							...
							if ($arResult['PROPERTIES']['additionalPhoto']['VALUE'])
				{
							foreach ($arResult['PROPERTIES']['additionalPhoto']['VALUE'] as $key => $imgId)
								{
									?>
									<div class="product-item-detail-slider-image<?=($key == 0 ? ' active' : '')?>" data-entity="image" data-id="<?=$imgId?>">
										<img src="<? echo CFile::GetPath($imgId); ?>" alt="<?=$alt?>" title="<?=$title?>"<?=($key == 0 ? ' itemprop="image"' : '')?> id='mainWindowImg'>
									</div>
									<?
								}
				}
							...
							if (!empty($actualItem['MORE_PHOTO']))
							{
								foreach ($actualItem['MORE_PHOTO'] as $key => $photo)
								{
									...
									<div class="product-item-detail-slider-image<?=($key == 0 ? ' active' : '')?>" data-entity="image" data-id="<?=$photo['ID']?>" id='<?=$photo['ID']?>'>
										<img src="<?=$photo['SRC']?>" alt="<?=$alt?>" title="<?=$title?>"<?=($key == 0 ? ' itemprop="image"' : '')?> id='mainWindowImg'>
									</div>
									<?
...
?>
									...
									<div class="product-item-detail-slider-image<?=($key == 0 ? ' active' : '')?>" data-entity="image" data-prev="<? echo $prevId; ?>" data-id="<? echo $imgId;?>" id="<? echo $imgId;?>">
										<img src="<? echo CFile::GetPath($imgId); ?>" alt="<?=$alt?>" title="<?=$title?>"<?=($key == 0 ? ' itemprop="image"' : '')?> id='mainWindowImg'>
									</div>
...
<?									$prevId = $imgId;
					...
					document.addEventListener('DOMContentLoaded', function(){ // Аналог $(document).ready(function(){
						var array = [];
						var array2 = {};
						$('#mainContainer div').each(function(key, item) {
							array.push(item);
						});
						...
						for (i = 0; i < length; ++i)
						{
							var id = array[i].id;
							array2[id] = [];
							var prev = array[i].getAttribute('data-prev');
							if (!!prev)
								array2[id]['prev'] = prev;
							else
								array2[id]['prev'] = '';
							...
							if (!!array[i + 1])
								array2[id]['next'] = array[i + 1].getAttribute('id');
						}
						...
						left.addEventListener('click', function() {
							var currentSlide = $('#mainContainer div.active');
							var currentSlideId = currentSlide.attr('id');
							var prevSlideId = this.getAttribute('data-slideid');
							if (!!prevSlideId)
							{
								right.setAttribute('data-slideid', currentSlideId);

								var prev = array2[prevSlideId]['prev'];
								if (!!prev)
									this.setAttribute('data-slideid', prev);
								else
									this.setAttribute('data-slideid', '');

								var slide = document.getElementById(prevSlideId);
								var nextSlideId = slide.getAttribute('data-prev');
						...
						right.addEventListener('click', function() {
							var current = $('#mainContainer div.active').attr('id');
							var nextSlideId = this.getAttribute('data-slideid');
							if (!!nextSlideId)
							{
								left.setAttribute('data-slideid', current);

								var next = array2[nextSlideId]['next'];
								if (!!next)
									this.setAttribute('data-slideid', next);
								else
									this.setAttribute('data-slideid', '');

								var slide = document.getElementById(nextSlideId);
								var prevSlideId = slide.getAttribute('data-prev');
								document.getElementById(prevSlideId).classList.remove('active');
								slide.classList.add('active');
							}
						});
					});
			...
			<div class="col-md-4 col-sm-12 product-item-detail">
				<div class="row">
					<div class="col-sm-12">
						<div class="product-item-detail-pay-block">
							<div class="product-item-detail-info-container" style='text-align: center; font-size: 20px; font-weight: 600'>
								<? echo $arResult['NAME']; ?>
							</div>
							<?
...
?>
		...
		<div class="row" style='margin-top: 23px;'>
			<div class="col-sm-12 col-md-12">
				<div class="row" id="<?=$itemIds['TABS_ID']?>" style='width: 99%'>
					<div class="col-xs-12">
...
$(document).ready(function(){
	...
	if ($('.swiper-slide').children().length < 3) {
