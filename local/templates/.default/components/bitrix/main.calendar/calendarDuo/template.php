...
<div class="col-xs-3">
	<div class="form-group pos-r">
		...
		?>
		<input type="text" class="form-control" id="<?=$arParams['INPUT_NAME'.($i == 1 ? '_FINISH' : '')]?>" name="<?=$arParams['INPUT_NAME'.($i == 1 ? '_FINISH' : '')]?>" value="<?=$arParams['INPUT_VALUE'.($i == 1 ? '_FINISH' : '')]?>" <?=(Array_Key_Exists("~INPUT_ADDITIONAL_ATTR", $arParams)) ? $arParams["~INPUT_ADDITIONAL_ATTR"] : ""?>/>
		<? } ?>
...
<?if ($cnt == 2 && $i == 0):?><div class="col-xs-1 text-center"><span class="date-interval-hellip">&hellip;</span></div><?endif;?>
<? } ?>
