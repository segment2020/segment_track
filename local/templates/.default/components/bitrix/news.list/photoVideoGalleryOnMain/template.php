...
<div class="block-title clearfix">
	<a class="notitlestyle" href="<? echo $arResult["SECTION_PAGE_URL"]; ?>"><? echo $arResult['NAME']; ?></a>
	<a class="floatright" href="<? echo $arResult['SECTION_PAGE_URL']; ?>">Все <? echo $linkText; ?><i class="icon-icons_main-10"></i></a>
</div>
<div class="row">
...
foreach($arResult["ITEMS"] as $itemkey => $arItem)
{
	...
	$dateCreate = FormatDate("d M Y h:i", MakeTimeStamp($arItem["DATE_CREATE"]));
?>

	<div class="col-sm-6 col-xs-12 <? if ($itemkey==0) { echo 'leftimg'; } else { echo 'rightimg'; } ?>" style="background-image: url('<? echo $arItem["PREVIEW_PICTURE"]["SRC"]; ?>');">
		<div class="mb_background">
			<? if ($arItem['DISPLAY_PROPERTIES']['companyId']['DISPLAY_VALUE']) 
			{ ?>
				<div class="firmblock">
					Альбом компании <? echo strip_tags($arItem['DISPLAY_PROPERTIES']['companyId']['DISPLAY_VALUE']); ?>
				</div>
			<? } ?>
