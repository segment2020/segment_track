...
<h1>#<?= $arResult["REQUEST"]["TAGS"] ?> </h1> 
<div class="block-default block-shadow content-margin hidden">
...
<div class="search-result__block block-default block-shadow content-margin">
    ...
    <?endif;?>
        <small><?= $arItem["DATE_CHANGE"] ?></small> 
    <!-- Вывод компании автора -->
        <?
            $res = GetIBlockElement($arItem["ITEM_ID"]);
            $companyLink = GetIBlockElement($res['PROPERTIES']['companyId']['VALUE'])
        ?>  
        <a href="<?echo $companyLink['DETAIL_PAGE_URL'] ?>" class='companyLink'>  
            <?echo $companyLink['NAME'] ?>
        </a> 
        <br />
    <hr />
</div>
