...
if (!empty($item['~PREVIEW_PICTURE']))
	$fileImg = CFile::ResizeImageGet($item['PREVIEW_PICTURE']["ID"], array('width'=>270, 'height'=>150), BX_RESIZE_IMAGE_PROPORTIONAL, true);
else
	$fileImg['src'] = '/tpl/images/160х160.jpg';

$res = CIBlockElement::GetByID($item['PROPERTIES']["companyId"]['VALUE']);
if ($ar_res = $res->GetNext())
	$companyName = $ar_res['NAME'];

$companyName = $item['DISPLAY_PROPERTIES']["companyId"]['DISPLAY_VALUE'];
?>
<div class="col-sm-4 col-xs-6 cell-12-xs content-margin">
	<div class="block-default block-shadow hitssecitem text-center">
		<div class="hitssecimg" style="background-image: url('<? echo !empty($fileImg['src'])? $fileImg['src']: '/tpl/images/no_image_160_160.jpg'; ?>');">
		...
		<div class="hitssectext">
			...
			<div class="hitssecfirm" style='color: #898989;'>
				<? echo !empty($companyName)? 'Хит компании ' . $companyName: ''; ?>
			</div>
			<div class="hitssecprice">
				<? echo $price['PRICE']; ?> <span>руб</span>
			</div>
			<div class="text-center buttonblock">
				<a class="btn minbr btn-blue-full" href="<? echo $item['DETAIL_PAGE_URL']; ?>">
					<span class="arrow text-center"><i class="icon-icons_main-10"></i></span><span class="text">Подробнее</span>
				</a>
			</div>
		</div>
	</div>
