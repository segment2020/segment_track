...
if ('1' === $arResult['PROPERTIES']['paidOption']['VALUE']) {
	$arSelect = array('PROPERTY_paidBrandsNum', 'PROPERTY_beginDatePB', 'PROPERTY_endDatePB');
	$arFilter = array("IBLOCK_ID" => IBLOCK_ID_COMPANY, 'ID' => $arResult['PROPERTIES']['companyId']['VALUE'], "ACTIVE" => "Y");
	$res = CIBlockElement::GetList(Array(), $arFilter, false, array(), $arSelect);
	if ($ob = $res->GetNextElement())
	{
		$arFields = $ob->GetFields();
		if ('' != $arFields['PROPERTY_PAIDBRANDSNUM_VALUE'])
		{
...
<div class="block-default in block-shadow content-margin detailblock clearfix">
	<div class="detailinfo clearfix">
		<div class="detailinfofirm floatleft">
			Собственник бренда <? echo $companyName; ?>
		</div>
<?
...
<?
	if ('1' === $arResult['PROPERTIES']['paidOption']['VALUE'] && $arResult["DETAIL_PICTURE"]["SRC"])
		$file = CFile::ResizeImageGet($arResult["DETAIL_PICTURE"]["ID"], array('width'=>890, 'height'=>340), BX_RESIZE_IMAGE_EXACT, true);
	else
		$file['src'] = '';

	if ('' != $file['src']) { ?>
		<div class="mainphoto" style="background-image: url('<?=$file["src"]?>');">
<?
...
<?
	if ('1' === $arResult['PROPERTIES']['paidOption']['VALUE'] && isset($arResult["DETAIL_TEXT"]) && !empty($arResult["DETAIL_TEXT"]))
	{
?>
