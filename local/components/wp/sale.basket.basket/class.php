	...
	public function getBasketItems()
	{
		...
		if (!empty($arElementId) && self::$catalogIncluded)
		{
			...
			while ($property = $propsIterator->GetNext())
			{
				...
				// Добавим ID компании в массив свойств товара.
				if ('companyId' == $property['CODE'])
				{
					$arSelect = Array("ID");
					$arFilter = Array("IBLOCK_ID" => IBLOCK_ID_COMPANY, "NAME" => $property['~VALUE']);
					...
					if ($ob = $res->GetNextElement())
					{
						...
						// pre('arFields');
						// pre($arFields);
						$property['COMPANY_ID'] = $arFields['ID'];
					}
					// pre($arFields['ID']);
				}
...
// pre($arBasketItems);
		...
		// pre($arOrder["BASKET_ITEMS"]);

		// Отсортируем по номеру компании.
		$tmpArr = array();
		$count = count($arOrder["BASKET_ITEMS"]) - 1;
		$cycle = true;
		while ($cycle) {
			$cycle = false;
			for ($i = 0; $i < $count; ++$i) {
				if ((int)$arOrder["BASKET_ITEMS"][$i]['PROPS'][0]['COMPANY_ID'] > (int)$arOrder["BASKET_ITEMS"][$i + 1]['PROPS'][0]['COMPANY_ID']) {
					$tmpArr = $arOrder["BASKET_ITEMS"][$i + 1];
					$arOrder["BASKET_ITEMS"][$i + 1] = $arOrder["BASKET_ITEMS"][$i];
					$arOrder["BASKET_ITEMS"][$i] = $tmpArr;
					$cycle = true;
				}
			}
		}
