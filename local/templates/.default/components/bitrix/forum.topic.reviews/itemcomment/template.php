...
?>
	...
	<div class="block-default block-shadow content-margin commentblock" id="<?=$arParams["FORM_ID"]?>container">
<?
		if ($arResult["NAV_RESULT"] && $arResult["NAV_RESULT"]->NavPageCount > 1)
			echo $arResult["NAV_STRING"]; ?>
...
?>
		<div class="clearfix" bx-author-id="<?=$res["AUTHOR_ID"]?>" bx-author-name="<?=$res["AUTHOR_NAME"]?>" id="message<?=$res["ID"]?>">
			...
			<div class="commentblocktext">
				<div class="commentblockname">
					<?
					// pre($res);
					echo $res['LAST_NAME'] . ' ' . $res['NAME'];
					/*
				...
				<div class="commentblockdate"><? echo $res["POST_DATE"]; ?></div>
				<div class="commentblockdescr" id="message_text_<?=$res["ID"]?>">
				...
				<div class="commentblockanswer">
					<noindex>
					<a href="#review_anchor" style='margin-left:0;' title="<?=GetMessage("REPLAY")?>" class="reviews-button-small" bx-act="reply"><?=GetMessage("REPLAY")?></a>
					</noindex>
				</div>
				<? } ?>
			</div>
